import {IAudioProvider, ITrackConstraint, IAudioTrack} from 'ionic-audio/dist/ionic-audio';
 ...
 * ````
 *
 * @element audio-track-play-stop
 * @parents audio-track
 * @export
 * @class AudioTrackPlayStopComponent
 */
@Component({
    selector: 'audio-track-play-stop',
    template: `
    <button clear (click)="toggle($event)" [disabled]="audioTrack.error || audioTrack.isLoading">
      <ion-icon name="square" *ngIf="audioTrack.isPlaying && !audioTrack.isLoading"></ion-icon>
...
export class AudioTrackPlayStopComponent {
